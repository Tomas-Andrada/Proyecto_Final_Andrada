    ...
    if blog.autor == request.user:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                form = BlogForm()
                blogs=Blog.objects.all()
                return render(request, 'BlogAndrada/detalle_blog.html', {'form': form, 'blogs': blogs, "mensaje": "Blog editado con exito"})
        else:
...
def eliminar_blog(request, blog_id):
    ...
    if blog.autor == request.user:
        if request.method == "POST":
            ...
            blogs = Blog.objects.all()
            return render(request, "BlogAndrada/detalle_blog.html", {"blogs": blogs, "mensaje": "Blog eliminado con éxito"})
        else:
